<script lang="ts">
  ...
  const summaryItems = $derived.by(() => {
    ...
    if (changes.newIssues.length > 0)
      items.push({ count: changes.newIssues.length, label: 'new', colorClass: 'text-change-new-text' });
    if (changes.removedIssues.length > 0)
      items.push({ count: changes.removedIssues.length, label: 'removed', colorClass: 'text-change-removed-text' });
    if (changes.statusChanges.length > 0)
      items.push({ count: changes.statusChanges.length, label: 'status changed', colorClass: 'text-change-status-text' });
    if (changes.commentChanges.length > 0)
      items.push({ count: changes.commentChanges.length, label: 'with new comments', colorClass: 'text-change-comments-text' });
    if (changes.assigneeChanges.length > 0)
      items.push({ count: changes.assigneeChanges.length, label: 'reassigned', colorClass: 'text-change-assignee-text' });
    return items;
  });
    ...
    {#if isExpanded}
      <div class="border-t border-primary/30 px-3 py-3 text-xs">
              ...
              <li class="flex items-start gap-3 py-1.5 hover:bg-background/30 rounded px-2 -mx-2">
                ...
                <div class="min-w-0 flex-1">
                  ...
                  <div class="flex items-center gap-2 mt-0.5 flex-wrap">
                    {#if issue.isNew}
                      <span class="text-[10px] font-data font-bold text-change-new-text bg-change-new/15 px-1.5 py-0.5 rounded">New</span>
                    {/if}
                    {#if issue.isRemoved}
                      <span class="text-[10px] font-data font-bold text-change-removed-text bg-change-removed/15 px-1.5 py-0.5 rounded">Removed</span>
                    {/if}
                    {#if issue.statusChange}
                      <span class="flex items-center gap-1 text-[10px] font-data">
                        <span class="text-muted-foreground line-through">{issue.statusChange.previousStatus}</span>
                        <AtlaskitIcon name="chevron-right" size={10} class="text-muted-foreground/50" />
                        <span class="font-bold text-change-status-text bg-change-status/15 px-1.5 py-0.5 rounded">{issue.statusChange.currentStatus}</span>
                      </span>
                    {/if}
                    {#if issue.commentChange}
                      <span class="text-[10px] font-data text-change-comments-text bg-change-comments/15 px-1.5 py-0.5 rounded"
                        >+{issue.commentChange.newCommentCount} comment{issue.commentChange.newCommentCount !== 1 ? 's' : ''}</span
                      >
                    {/if}
                    {#if issue.assigneeChange}
                      <span class="flex items-center gap-1 text-[10px] font-data">
                        <span class="text-muted-foreground">{issue.assigneeChange.previousAssignee ?? 'Unassigned'}</span>
                        <AtlaskitIcon name="chevron-right" size={10} class="text-muted-foreground/50" />
                        <span class="text-change-assignee-text font-bold">{issue.assigneeChange.currentAssignee ?? 'Unassigned'}</span>
                      </span>
                    {/if}
                  </div>
                </div>
        ...
        {:else}
          ...
          {#if changes.newIssues.length > 0}
            <div>
              <div class="flex items-center gap-2 text-change-new-text bg-change-new/10 px-2 py-1 rounded text-xs font-bold uppercase tracking-wider mb-2">
              ...
              <ul class="space-y-0.5">
                {#each changes.newIssues as issue (issue.key)}
                  <li class="flex items-start gap-3 py-1.5 hover:bg-background/30 rounded px-2 -mx-2">
                    ...
                    <span class="text-sm font-medium text-foreground/80 truncate">{issue.summary}</span>
                  </li>
                {/each}
              </ul>
            </div>
          ...
          {#if changes.removedIssues.length > 0}
            <div>
              <div class="flex items-center gap-2 text-change-removed-text bg-change-removed/10 px-2 py-1 rounded text-xs font-bold uppercase tracking-wider mb-2">
              ...
              <ul class="space-y-0.5">
                {#each changes.removedIssues as issue (issue.key)}
                  <li class="flex items-start gap-3 py-1.5 hover:bg-background/30 rounded px-2 -mx-2">
                    ...
                    <div class="min-w-0 flex-1">
                      <span class="text-sm font-medium text-foreground/80 line-through">{issue.summary}</span>
                      <span class="text-[10px] font-data text-muted-foreground ml-2">(was: {issue.lastStatus})</span>
                    </div>
                  </li>
                {/each}
              </ul>
          ...
          {#if changes.statusChanges.length > 0}
            <div>
              <div class="flex items-center gap-2 text-change-status-text bg-change-status/10 px-2 py-1 rounded text-xs font-bold uppercase tracking-wider mb-2">
              ...
              <ul class="space-y-0.5">
                {#each changes.statusChanges as change (change.key)}
                  <li class="flex items-start gap-3 py-1.5 hover:bg-background/30 rounded px-2 -mx-2">
                    ...
                    <div class="min-w-0 flex-1">
                      <span class="text-sm font-medium text-foreground/80 truncate">{change.summary}</span>
                      <div class="flex items-center gap-1.5 mt-0.5">
                        <span class="text-[10px] font-data text-muted-foreground line-through">{change.previousStatus}</span>
                        <AtlaskitIcon name="chevron-right" size={10} class="text-muted-foreground/50" />
                        <span class="text-[10px] font-data font-bold text-change-status-text bg-change-status/15 px-1.5 py-0.5 rounded">{change.currentStatus}</span>
                      </div>
                    </div>
                  </li>
                {/each}
          ...
          {#if changes.commentChanges.length > 0}
            <div>
              <div class="flex items-center gap-2 text-change-comments-text bg-change-comments/10 px-2 py-1 rounded text-xs font-bold uppercase tracking-wider mb-2">
                ...
                {#each changes.commentChanges as change (change.key)}
                  <li class="flex items-start gap-3 py-1.5 hover:bg-background/30 rounded px-2 -mx-2">
                    ...
                    <div class="min-w-0 flex-1">
                      ...
                      <div class="mt-0.5">
                        <span class="text-[10px] font-data text-change-comments-text bg-change-comments/15 px-1.5 py-0.5 rounded">
                          +{change.newCommentCount}{change.newCommentCount === 1 && change.latestAuthor ? ` by ${change.latestAuthor}` : ''}
                        </span>
                      </div>
                    </div>
                  </li>
          ...
          {#if changes.assigneeChanges.length > 0}
            <div>
              <div class="flex items-center gap-2 text-change-assignee-text bg-change-assignee/10 px-2 py-1 rounded text-xs font-bold uppercase tracking-wider mb-2">
                ...
                {#each changes.assigneeChanges as change (change.key)}
                  <li class="flex items-start gap-3 py-1.5 hover:bg-background/30 rounded px-2 -mx-2">
                    ...
                    <div class="min-w-0 flex-1">
                      <span class="text-sm font-medium text-foreground/80 truncate">{change.summary}</span>
                      <div class="flex items-center gap-1.5 mt-0.5">
                        <span class="text-[10px] font-data text-muted-foreground">{change.previousAssignee ?? 'Unassigned'}</span>
                        <AtlaskitIcon name="chevron-right" size={10} class="text-muted-foreground/50" />
                        <span class="text-[10px] font-data font-bold text-change-assignee-text">{change.currentAssignee ?? 'Unassigned'}</span>
                      </div>
                    </div>
                  </li>
                {/each}
