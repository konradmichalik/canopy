...
// Pre-encoded badge SVG (red dot, top-right corner)
const BADGE_CIRCLE = encodeURIComponent('<circle cx="900" cy="100" r="100" fill="#bf616a"/>');

// Cache the badged favicon data URL
let badgedFaviconUrl: string | null = null;

/**
 * Set the Tauri app badge (macOS dock icon)
 * @param count - Number to display, or 0/undefined to clear
 */
async function setTauriBadge(count: number | undefined): Promise<void> {
  ...
  try {
    ...
    await invoke('set_app_badge', { count: count && count > 0 ? count : null });
  } catch (error) {
    // Silently fail - badge is a nice-to-have feature
    console.warn('Failed to set Tauri badge:', error);
  }
}

/**
 * Set the favicon with or without a notification badge
 * @param showBadge - Whether to show the badge indicator
 * @param changeCount - Number of changes (for Tauri dock badge)
 */
export function setFaviconBadge(showBadge: boolean, changeCount?: number): void {
